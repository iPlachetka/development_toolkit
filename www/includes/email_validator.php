...
class Validate_Email {
  ...
  */
 function Validate_Email($emails = false, $sender = false) {
  if ($emails) {
 ...
 function queryMX($domain) {
	...
	$mxweights = array();
 	if (function_exists('getmxrr')) {  
		...
		$this->win_getmxrr($domain, $hosts, $mxweights);
 	}
	return array($hosts, $mxweights);
 } 

	// getmxrr() support for Windows by HM2K <php [spat] hm2k.org> 
   function win_getmxrr($domain, &$hosts, &$mxweights=false) {
	    if (strtoupper(substr(PHP_OS, 0, 3)) != 'WIN') return;
	    if (!is_array ($hosts) ) $hosts = array();
	    if (empty($domain)) return;
	    $exec='nslookup -type=MX '.escapeshellarg($domain);
	    @exec($exec, $output);
	    if (empty($output)) return;
	    $i=-1;
	    foreach ($output as $line) {
	        $i++;
	        if (preg_match("/^$domain\tMX preference = ([0-9]+), mail exchanger = (.+)$/i", $line, $parts)) {
	          $mxweights[$i] = trim($parts[1]);
	          $hosts[$i] = trim($parts[2]);
	        }
	        if (preg_match('/responsible mail addr = (.+)$/i', $line, $parts)) {
	          $mxweights[$i] = $i;
	          $hosts[$i] = trim($parts[1]);
	        }
	    }
	    return ($i!=-1);
	}  
...
$emails = array(
		'maurice.calhoun@usg.edu',
		'mauricecalhoun@gmail.com',
		'me@mauricecalhoun.com',
		'mauricecalhoun@echo45.com',
		'jason@echo45.com'
); 
...
// {
// $datestring = "Last Updated on %M %d, %Y at %g:%i %A";
// echo "<p>" . mdate($datestring, $time) . "</p>";   
// }
// EXIT;
// an optional sender
// $sender = 'me@mauricecalhoun.com';
...
//  	$email_results['valid'][] = $email;
//   } else {
//     //echo "<p>The email address {$email} is not valid </p>";
// 	$email_results['invalid'][] = $email;    
//   }
// }

//$start = $validator->microtime_float(); 
//$validator->debuger($email_results);
//$end = $validator->microtime_float();  
